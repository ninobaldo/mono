...
O próximo passo a ser tomado deve ser recuperar todos os arquivos do disco, isso incluí todos os arquivos \emph{Master Table File (MFT)}, deletados ou não, \emph{data carving} por arquivos com assinaturas de binário... data carve, fragmentos de arquivos, data streams alternativos (feature stream do NTFS), slack space e arquivos ocultos;



Todos os arquivos tem um especificação que define o formato que o arquivo deve ter para ser corretamente usado, esse formato é usado por ferramentas para, por exemplo a especificação do \emph{Portable Executable (PE) File Format} \cite{MS2013} determina que todos os executáveis comecem com um pequeno executável MS-DOS que inicia com o \emph{Magic Number} 0x5A4D ou MZ em ASCII que são as iniciais de Mark Zbikowski um dos arquitetos originais do MS-DOS \cite{PIETREK2002}, a função desse pequeno programa é apresentar a mensagem ``This program cannot be run in DOS mode". O rootkit pode usar programas como o \emph{Transmogrify} que é capaz de mascarar um arquivo em qualquer assinatura \cite{MAYNOR2007}.

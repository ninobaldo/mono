...
\begin{itemize}
  ...
  \item Hardware/Firmware: os rootkit se instalam em \emph{firmwares} como os \emph{firmwares} das placas de rede ou placas de vídeo, de forma que rodam indepententes do sistema operacional; e
...
Mater o controle sobre o usuário permite ao rootkit continuar escondido. \citeonline{BUTLER2006} mostra em seu trabalho técnicas para manter o rootkit oculto, para isso o mesmo deve ganhar controle sobre a execução e/ou manipular o dados do \emph{kernel}.
...
Apesar cada linguagem de programação possui métodos ou \emph{functions} próprias para criar um arquivo, cada qual com sua sintaxe e pecularidades, contudo é bem provável que sejam apenas outra abstração e que na realidade o mesmo use a \emph{function \textbf{CreateFile}} dispinibilizada pelo \emph{Windows API} \cite{MARK2012}.

\emph{Native system services ou system calls} são serviços do sistema operacional não documentados que são disponibilizados para uso em \emph{user-mode}, sendo inclusive chamados por \emph{functions} do \emph{Windows API}, por exemplo, \emph{\textbf{NtCreateUserProcess}} é o serviço interno do sistema que a \emph{function} \emph{\textbf{CreateProcess}} do \emph{Windows API} chama para criar novos processos. Já as chamadas \emph{Kernel support functions ou routines} também são subrotinas do sistema operacional Microsoft Windows, contudo essas são apenas chamadas em \emph{kernel-mode} \cite[p. 4]{MARK2012}.
